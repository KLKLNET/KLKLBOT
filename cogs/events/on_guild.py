...
class Guild(Cog):
    ...
    async def on_guild_add(self, guild):
        try:
            ...
            embed.add_field(
                name="**__Members__**",
                value=f"""🔄 Members : {len(guild.members)}\n 👨🏻‍💼ns : {len(list(filter(lambda m: not m.bot, guild.members)))}\n🤖Bots : {len(list(filter(lambda m: m.bot, guild.members)))}
                """,
                inline=False
            )
            embed.add_field(
...
Threads : {len(guild.threads)}
            ...
            embed = discord.Embed(description="➡️Prefix For This Server is `!`\n➡️ Get Started with `!help`\n➡️For detailed guides, FAQ & information, visit our **[Support Server](https://discord.gg/7Ydjf6874w)**",
    color=0xff0000)
            embed.set_author(name="Thanks for adding me!", icon_url=guild.me.display_avatar.url)
            embed.set_footer(text="Powered by KLKL Development™",)
            if guild.icon:
                embed.set_thumbnail(url=guild.icon.url)
