...
class Owner(commands.Cog):
    ...
    def __init__(self, client):
        ...
        self.bot_owner_ids = [1360373818148913362,]
    ...
    async def staff_add(self, ctx, user: discord.User):
        if user.id in self.staff:
            sonu = discord.Embed(title="⚠️ Access Denied", description=f"{user} is already in the staff list.", color=0x000000)
    ...
    async def staff_remove(self, ctx, user: discord.User):
        if user.id not in self.staff:
            sonu = discord.Embed(title="⚠️ Access Denied", description=f"{user} is not in the staff list.", color=0x000000)
            await ctx.reply(embed=sonu, mention_author=False)
        else:
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            sonu = discord.Embed(title="✅ KLKL Staffs", description=f"\n{staff_display}", color=0x000000)
    ...
    async def _slist(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of KLKL [{len(self.client.guilds)}]",
            color=0x000000,
            per_page=10),
            ctx=ctx)
    ...
    async def _restart(self, ctx: Context):
        await ctx.reply("Restarting KLKL...")
        restart_program()
    ...
    async def own_list(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            title=f"KLKL Owners [{len(nplist)}]",
            description="",
            per_page=10,
            color=0x000000),
                              ctx=ctx)
