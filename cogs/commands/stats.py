...
class Stats(commands.Cog):
    ...
    async def stats(self, ctx):
        processing_message = await ctx.send("⏳ Loading KLKL information...")
        ...
        embed = Embed(title="KLKL Statistics: General", color=0x000000)
        ...
        embed.add_field(name="🎵 Music Stats", value=f"Currently Connected: **{channels_connected}**\nCurrently Playing: **{playing_tracks}**\nTotal Songs Played: **{self.total_songs_played}**", inline=False)
        embed.set_footer(text="Powered by KLKL Development™", icon_url=self.bot.user.display_avatar.url)
        ...
        async def system_button_callback(interaction):
            if interaction.user == ctx.author:
                system_embed = Embed(title="KLKL Statistics: System", color=0x000000)
                system_embed.add_field(name="⚙️ System Info", value=f"• Discord.py: **{discord.__version__}**\n• Python: **{platform.python_version()}**\n• Architecture: **{platform.machine()}**\n• Platform: **{platform.system()}**", inline=False)
                ...
                system_embed.set_footer(text="Powered by KLKL Development™", icon_url=self.bot.user.display_avatar.url)
        ...
        async def ping_button_callback(interaction):
            if interaction.user == ctx.author:
                ...
                ping_embed.set_footer(text="Powered by KLKL Development™", icon_url=self.bot.user.display_avatar.url)
