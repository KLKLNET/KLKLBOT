...
class MusicControlView(View):
    ...
    @discord.ui.button(emoji="🔊", style=discord.ButtonStyle.secondary, label="Up Volume")
    async def increase_volume_button(self, interaction: discord.Interaction, button: Button):
        if self.player and self.player.playing:
            current_volume = self.player.volume  # الحصول على المستوى الحالي
            new_volume = min(150, current_volume + 10)  # زيادة بـ 10%
            await self.player.set_volume(new_volume)
            await interaction.response.send_message(f"Volume increased to {new_volume}% by **{interaction.user.display_name}**.", ephemeral=True)
        else:
            await interaction.response.send_message("No track is currently playing.", ephemeral=True)
    # زر لتقليل الصوت (اللي كان موجود)
    @discord.ui.button(emoji="🔉", style=discord.ButtonStyle.secondary, label="Down Volume")
    async def decrease_volume_button(self, interaction: discord.Interaction, button: Button):
        if self.player and self.player.playing:
            current_volume = self.player.volume
            new_volume = max(1, current_volume - 10)  # تقليل بـ 10%
            await self.player.set_volume(new_volume)
            await interaction.response.send_message(f"Volume decreased to {new_volume}% by **{interaction.user.display_name}**.", ephemeral=True)
        else:
            await interaction.response.send_message("No track is currently playing.", ephemeral=True)       
...
class Music(commands.Cog):
        ...
        if len(guild.voice_channels[0].members) == 1:
            ...
            if player:
                ...
                try:
                    ...
                    ended.set_footer(text="Thanks for choosing KLKL!")
                    support = Button(label='Support',
                                 style=discord.ButtonStyle.link,
                        url=f'https://discord.gg/7Ydjf6874w')
                    vote = Button(label='Vote',
                                 style=discord.ButtonStyle.link,
                        url=f'https://top.gg/bot/')
                    view = View()
    ...
    async def display_player_embed(self, player, track, ctx, autoplay=False):
        if track.artwork:
            ...
            embed.set_author(name="Now Playing", icon_url="https://cdn.discordapp.com/attachments/1428482153410330744/1429838189312475257/giphy.gif?ex=68f79815&is=68f64695&hm=ec040eae64b43cc3a420e9fe3d263b625510a7fc33e28ef80572f793d6e4df29&")
            embed.add_field(name="Author", value=f"`{track.author}`")
            embed.add_field(name="Duration", value=f"`{duration}`")
            embed.add_field(name="Source", value=f"[<:spotify:1429850392325390436>  Listen on Spotify]({track.uri})" if "spotify" in track.source else f"[<:jiosaavn:1429850387065737328> Listen on JioSaavn]({track.uri})" if "jiosaavn" in track.source else f"[<:SoundCloud:1307002774738829413> Listen on SoundCloud]({track.uri})" if "soundcloud" in track.source else f"[<:YouTube:1429850389552828587> Listen on YouTube]({track.uri})")
    ...
    async def play_source(self, ctx, query):
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(embed=discord.Embed(description=f"➕ Added playlist [{tracks.name}](https://discord.gg/7Ydjf6874w) with **{len(tracks.tracks)} songs** to the queue.", color=0x000000))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(embed=discord.Embed(description=f"➕  Added [{track.title}](https://discord.gg/7Ydjf6874w) to the queue.", color=0x000000))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(embed=discord.Embed(description=f"➕ Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://discord.gg/7Ydjf6874w)** to the queue.", color=0x000000))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"➕ Added all tracks from album **[{album_info['name']}](https://discord.gg/7Ydjf6874w)** to the queue.", color=0x000000))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description=f"🎙️ Volume set to {level}% by {ctx.author.mention}.", color=0x000000))
            else:
