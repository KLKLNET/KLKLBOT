    ...
    async def callback(self, interaction: discord.Interaction):
        if self.invoker == interaction.user:
            ...
            await interaction.response.send_message(
                "⚠️ You must run this command to interact with it.", ephemeral=True)
...
class View(discord.ui.View):
    ...
    def add_buttons(self):
        self.homeB = Button(label="", style=discord.ButtonStyle.secondary, emoji="⏮️", command=self.set_page, args=0, ctx=self.ctx)
        self.backB = Button(label="", style=discord.ButtonStyle.secondary, emoji="⬅️", command=self.to_page, args=-1, ctx=self.ctx)
        self.quitB = Button(label="", style=discord.ButtonStyle.danger, emoji="🗑️", command=self.quit, ctx=self.ctx)
        self.nextB = Button(label="", style=discord.ButtonStyle.secondary, emoji="➡️", command=self.to_page, args=1, ctx=self.ctx)
        self.lastB = Button(label="", style=discord.ButtonStyle.secondary, emoji="⏭️", command=self.set_last_page, ctx=self.ctx)
